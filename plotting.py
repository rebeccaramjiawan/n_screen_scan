...
def plot_btv(btv_calc_inst, btv, btvs, gamma_r, estimates, data_xy):
    ...
    for i in range(len(btv)):
        ...
        if len(btv) > 3:
            table_values = np.vstack((table_values, [r"$D_x$", np.round(disp_x, 3), "m"], [r"$D_y$", np.round(disp_y, 3), "m"]))
        if len(btv) > 4:
            table_values = np.vstack((table_values, [r"$D'_x$", np.round(deriv_disp_x, 3), "m"], [r"$D'_y$", np.round(deriv_disp_y, 3), "m"]))
        if len(btv) > 5:
        ...
        xmin_int = np.ceil(xmin)
        ymin_int = np.ceil(ymin)
        xmax_int = np.floor(xmax)
        ymax_int = np.floor(ymax)
        ...
        g = sns.heatmap(np.transpose(data_xy[btv[i]]["image"]), yticklabels=y_axis_labels, cbar=False, cmap="mako",
                        ax=ax[1][0])
        ...
        try:
            ax[0][0].text(0.75, 0.15, "Mean = %s $\mu$m \n R.M.S = %s $\mu$m \n Amp. %s a.u" % (
                (np.round(data_xy[btv[i]]["mu_x"], 2)), np.round(data_xy[btv[i]]["sigma_x"] * 10 ** 3, 2),
                data_xy[btv[i]]["amp"]),
        ...
        try:
            ax[1][1].text(-0.05, -0.2, "Mean = %s $\mu$m \n R.M.S = %s $\mu$m" % (
                (np.round(data_xy[btv[i]]["mu_y"], 2)), np.round(data_xy[btv[i]]["sigma_y"] * 10 ** 3, 2)), fontsize=20,
                          transform=ax[1][1].transAxes)
        except:
            pass

        table = ax[1][2].table(cellText=table_values, colLabels=[r"$\bf{Parameter}$", r"$\bf{Value}$", r"$\bf{Units}$"],
